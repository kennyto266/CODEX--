...
class TencentQuantAnalyzer:
    ...
    def calculate_volatility(self):
        ...
        # 歷史波動率 (年化)
        daily_vol = self.data['daily_return'].std()
        annual_vol = daily_vol * np.sqrt(252)  # 252個交易日
        
        # 近期波動率 (最近10天)
        recent_vol = self.data['daily_return'].tail(10).std() * np.sqrt(252)
    ...
    def analyze_trend(self):
        ...
        prev_5d = self.data.iloc[-6]
        prev_10d = self.data.iloc[-11] if len(self.data) >= 11 else self.data.iloc[0]
        ...
        # MA趨勢判斷
        ma5_trend = "上升" if latest['MA5'] > self.data.iloc[-2]['MA5'] else "下降"
        ma20_trend = "上升" if latest['MA20'] > self.data.iloc[-2]['MA20'] else "下降"
    ...
    def analyze_volume(self):
        ...
        # 成交量相對平均值
        volume_vs_avg = (latest['volume'] - latest['Volume_MA']) / latest['Volume_MA'] * 100
        
        # 最近5天成交量趨勢
        recent_volumes = self.data.tail(5)['volume'].tolist()
        volume_trend = "增加" if recent_volumes[-1] > recent_volumes[0] else "減少"
        
        # 價量配合度
        price_change_5d = (self.data.iloc[-1]['close'] - self.data.iloc[-6]['close']) / self.data.iloc[-6]['close']
        ...
        return {
            ...
            'average_volume': int(latest['Volume_MA'])
        }
    ...
    def technical_signals(self):
        ...
        # MACD信號
        if latest['MACD'] > latest['MACD_signal'] and self.data.iloc[-2]['MACD'] <= self.data.iloc[-2]['MACD_signal']:
            signals.append({"type": "MACD", "signal": "金叉", "strength": "強", "description": "MACD線向上穿越信號線"})
        elif latest['MACD'] < latest['MACD_signal'] and self.data.iloc[-2]['MACD'] >= self.data.iloc[-2]['MACD_signal']:
    ...
    def risk_assessment(self):
        ...
        # 技術風險
        rsi_risk = "高" if latest['RSI'] > 70 or latest['RSI'] < 30 else "中" if latest['RSI'] > 60 or latest['RSI'] < 40 else "低"
    ...
    def investment_recommendation(self):
        ...
        # 趨勢評分
        if trend['short_term_trend'] > 5: bullish_signals += 2
        elif trend['short_term_trend'] < -5: bearish_signals += 2
        
        if trend['mid_term_trend'] > 10: bullish_signals += 3
        elif trend['mid_term_trend'] < -10: bearish_signals += 3
        
        # 成交量評分
        if volume['price_volume_sync'] == '良好': bullish_signals += 1
        ...
        analysis = {
            ...
            'technical_analysis': {
                ...
                'technical_indicators': {
                    'RSI': round(self.data.iloc[-1]['RSI'], 2),
                    'MACD': round(self.data.iloc[-1]['MACD'], 2),
                    'MACD_signal': round(self.data.iloc[-1]['MACD_signal'], 2),
                    'MA5': round(self.data.iloc[-1]['MA5'], 2),
                    'MA10': round(self.data.iloc[-1]['MA10'], 2),
                    'MA20': round(self.data.iloc[-1]['MA20'], 2),
                    'BB_upper': round(self.data.iloc[-1]['BB_upper'], 2),
                    'BB_lower': round(self.data.iloc[-1]['BB_lower'], 2)
                },
                'signals': self.technical_signals()
            },
            ...
            'detailed_analysis': {
                ...
                'recent_performance': '近期股價表現強勁，突破前期整理區間',
