...
class RandomForestModel(BaseMLModel):
    ...
    def __init__(self, name: str = "RandomForest", task_type: str = 'regression',
    ...
    def _build_model(self) -> RandomForestRegressor | RandomForestClassifier:
        """
        构建随机森林模型
    ...
    def train(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            # 训练模型
            logger.info(f"开始训练随机森林模型 (n_estimators={self.n_estimators})")
            self.model.fit(X, y)
            ...
            if self.task_type == 'regression':
                ...
                logger.info(f"随机森林训练完成 - Train MSE: {train_mse:.6f}, MAE: {train_mae:.6f}")
                ...
                logger.info(f"随机森林训练完成 - Train Accuracy: {train_acc:.4f}")
