...
class HKEXAdapter(BaseDataAdapter):
    ...
    async def fetch_data(
        ...
        **kwargs
        ...
        logger.info(f"获取港交所交易量数据: {start_date} 到 {end_date}")
        ...
        try:
            ...
            for item in data:
                data_points.append(NonPriceDataPoint(
                    ...
                    quality_score=item.get('quality_score', 0.98)
                ))

            logger.info(f"成功获取 {len(data_points)} 条港交所交易量数据")
            return data_points

        except Exception as e:
            logger.error(f"获取港交所交易量数据失败: {str(e)}")
            raise
    ...
    async def _fetch_mock_data(
        ...
        for i, dt in enumerate(dates):
            ...
            # 模拟交易量波动
            daily_turnover = base_turnover + np.random.normal(0, 300)
            daily_turnover = max(500, daily_turnover)

            data.append({
                'timestamp': datetime.combine(dt.date(), datetime.min.time()),
                'value': round(daily_turnover, 2),
                'quality_score': 0.95
            })

        logger.debug(f"生成 {len(data)} 条模拟港交所交易量数据")
        return data
    ...
    async def _fetch_real_data(self, start_date: date, end_date: date) -> List[Dict[str, Any]]:
        ...
        try:
            # 读取真实数据文件
            data_file = Path(__file__).parent.parent.parent.parent.parent / "data" / "real_gov_data" / "hkex_data_2025_11.json"
            ...
            for item in real_data:
                ...
                if start_date <= item_date <= end_date:
                    filtered_data.append({
                        ...
                        'value': item['daily_turnover_hkd_billions'],
                        'quality_score': item.get('quality_score', 0.98)
                    })

            logger.info(f"成功加载 {len(filtered_data)} 条真实港交所交易量数据")
            return filtered_data

        except Exception as e:
            logger.error(f"读取真实港交所交易量数据失败: {str(e)}, 使用模拟数据")
            return await self._fetch_mock_data(start_date, end_date)
    ...
    def validate_data(self, data: List[NonPriceDataPoint]) -> DataValidationResult:
        ...
        if min_val < 0:
            errors.append(f"交易量不能为负数: {min_val}")
    ...
    def to_ohlcv(self, data: List[NonPriceDataPoint]) -> pd.DataFrame:
        ...
        df['volume'] = (df['quality_score'] * 10000000).round().astype(int)

        result = df[['timestamp', 'open', 'high', 'low', 'close', 'volume']].copy()
        result['timestamp'] = pd.to_datetime(result['timestamp'])

        logger.debug(f"转换 {len(data)} 条港交所交易量数据为OHLCV格式")
        return result
