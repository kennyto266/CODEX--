...
class VisitorAdapter(BaseDataAdapter):
    ...
    async def fetch_data(
        ...
        **kwargs
        ...
        logger.info(f"获取访客数据: {start_date} 到 {end_date}")
        ...
        try:
            ...
            logger.info(f"成功获取 {len(data_points)} 条访客数据")
            return data_points

        except Exception as e:
            logger.error(f"获取访客数据失败: {str(e)}")
            raise
    ...
    async def _fetch_mock_data(
        ...
        for i, dt in enumerate(dates):
            # 模拟季节性（7-10月访客较多）
            seasonal = 0.3 * np.sin(2 * np.pi * (i % 12) / 12)
            trend = 0.001 * i  # 轻微增长趋势
            noise = np.random.normal(0, 0.1)
            
            visitors = base_visitors * (1 + seasonal + trend + noise)
            visitors = max(100000, visitors)  # 最小值

            data.append({
                'timestamp': datetime.combine(dt.date(), datetime.min.time()),
                'value': round(visitors),
                'quality_score': 0.95
            })

        logger.debug(f"生成 {len(data)} 条模拟访客数据")
        return data
    ...
    async def _fetch_real_data(self, start_date: date, end_date: date) -> List[Dict[str, Any]]:
        ...
        try:
            # 读取真实数据文件
            data_file = Path(__file__).parent.parent.parent.parent.parent / "data" / "real_gov_data" / "visitor_data_2025_11.json"
            ...
            for item in real_data:
                ...
                if start_date <= item_date <= end_date:
                    filtered_data.append({
                        ...
                        'quality_score': item.get('quality_score', 0.94)
                    })

            logger.info(f"成功加载 {len(filtered_data)} 条真实访客数据")
            return filtered_data

        except Exception as e:
            logger.error(f"读取真实访客数据失败: {str(e)}, 使用模拟数据")
            return await self._fetch_mock_data(start_date, end_date)
    ...
    def validate_data(self, data: List[NonPriceDataPoint]) -> DataValidationResult:
        ...
        if min_val < 0:
            errors.append(f"访客数不能为负数: {min_val}")
    ...
    def to_ohlcv(self, data: List[NonPriceDataPoint]) -> pd.DataFrame:
        ...
        logger.debug(f"转换 {len(data)} 条访客数据为OHLCV格式")
        return result
