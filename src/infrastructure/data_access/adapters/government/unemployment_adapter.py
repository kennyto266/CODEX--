...
class UnemploymentAdapter(BaseDataAdapter):
    """
    失业率数据适配器
    ...
    async def fetch_data(
        ...
        **kwargs
        ...
        logger.info(f"获取失业率数据: {start_date} 到 {end_date}")
        ...
        try:
            ...
            logger.info(f"成功获取 {len(data_points)} 条失业率数据")
            return data_points

        except Exception as e:
            logger.error(f"获取失业率数据失败: {str(e)}")
            raise
    ...
    async def _fetch_mock_data(
        ...
        for i, dt in enumerate(dates):
            # 模拟失业率的周期性（经济衰退时上升）
            cyclical = 2.0 * np.sin(2 * np.pi * i / 48)  # 4年周期
            trend = 0.01 * i  # 轻微上升趋势
            noise = np.random.normal(0, 0.2)

            unemployment_rate = base_rate + cyclical + trend + noise
            unemployment_rate = max(1.0, min(10.0, unemployment_rate))
        ...
        logger.debug(f"生成 {len(data)} 条模拟失业率数据")
        return data
    ...
    async def _fetch_real_data(self, start_date: date, end_date: date) -> List[Dict[str, Any]]:
        ...
        try:
            # 读取真实数据文件
            data_file = Path(__file__).parent.parent.parent.parent.parent / "data" / "real_gov_data" / "unemployment_data_2025_11.json"
            ...
            for item in real_data:
                ...
                if start_date <= item_date <= end_date:
                    filtered_data.append({
                        ...
                        'value': item['unemployment_rate'],
                        'quality_score': item.get('quality_score', 0.96)
                    })

            logger.info(f"成功加载 {len(filtered_data)} 条真实失业率数据")
            return filtered_data

        except Exception as e:
            logger.error(f"读取真实失业率数据失败: {str(e)}, 使用模拟数据")
            return await self._fetch_mock_data(start_date, end_date)
    ...
    def validate_data(self, data: List[NonPriceDataPoint]) -> DataValidationResult:
        ...
        if min_val < 0:
            errors.append(f"失业率不能为负数: {min_val}")
        if max_val > 20:
            warnings.append(f"失业率较高: {max_val}%")
    ...
    def to_ohlcv(self, data: List[NonPriceDataPoint]) -> pd.DataFrame:
        ...
        logger.debug(f"转换 {len(data)} 条失业率数据为OHLCV格式")
        return result
