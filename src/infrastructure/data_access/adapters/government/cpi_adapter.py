...
class CPIAdapter(BaseDataAdapter):
    """
    CPI数据适配器
    ...
    async def fetch_data(
        ...
        **kwargs
        ...
        logger.info(f"获取CPI指数数据: {start_date} 到 {end_date}")
        ...
        try:
            ...
            for item in data:
                data_points.append(NonPriceDataPoint(
                    ...
                    metadata={
                        'base_period': item.get('base_period', '2019=100'),
            ...
            logger.info(f"成功获取 {len(data_points)} 条CPI指数数据")
            return data_points

        except Exception as e:
            logger.error(f"获取CPI指数数据失败: {str(e)}")
            raise
    ...
    async def _fetch_mock_data(
        ...
        for i, dt in enumerate(dates):
            ...
            cyclical = 0.03 * np.sin(2 * np.pi * i / 12)  # 年度周期
            noise = np.random.normal(0, 0.01)

            index_value = base_index * (1 + trend + cyclical + noise)
            index_value = max(80, index_value)
        ...
        logger.debug(f"生成 {len(data)} 条模拟CPI指数数据")
        return data
    ...
    async def _fetch_real_data(self, start_date: date, end_date: date) -> List[Dict[str, Any]]:
        ...
        try:
            # 读取真实数据文件
            data_file = Path(__file__).parent.parent.parent.parent.parent / "data" / "real_gov_data" / "cpi_data_2025_11.json"
            ...
            for item in real_data:
                ...
                if start_date <= item_date <= end_date:
                    filtered_data.append({
                        ...
                        'value': item['cpi_value'],
                        'base_period': item.get('base_period', '2019=100'),
                        'quality_score': item.get('quality_score', 0.97)
                    })

            logger.info(f"成功加载 {len(filtered_data)} 条真实CPI数据")
            return filtered_data

        except Exception as e:
            logger.error(f"读取真实CPI数据失败: {str(e)}, 使用模拟数据")
            return await self._fetch_mock_data(start_date, end_date)
    ...
    def validate_data(self, data: List[NonPriceDataPoint]) -> DataValidationResult:
        ...
        if min_val < 0:
            errors.append(f"CPI指数不能为负数: {min_val}")

        if max_val > 200:
            warnings.append(f"CPI指数较高: {max_val}")
    ...
    def to_ohlcv(self, data: List[NonPriceDataPoint]) -> pd.DataFrame:
        ...
        logger.debug(f"转换 {len(data)} 条CPI指数数据为OHLCV格式")
        return result
