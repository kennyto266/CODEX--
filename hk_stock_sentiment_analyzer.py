...
class HKStockSentimentAnalyzer:
    ...
    def calculate_technical_indicators(self) -> Dict[str, Any]:
        ...
        # 模擬30日價格數據（基於實際變化）
        np.random.seed(42)  # 確保結果一致性
        daily_returns = np.random.normal(0.002, 0.03, self.analysis_period)
        daily_returns[-1] = self.change_percentage / 100  # 最後一日為實際變化
        ...
        # 計算技術指標
        sma_5 = np.mean(prices[-5:]) if len(prices) >= 5 else self.current_price
        sma_20 = np.mean(prices[-20:]) if len(prices) >= 20 else self.current_price
