...
def print_summary(results: List[Dict], symbol: str):
    ...
    print("📋 策略優化結果 (前10名)")
    print("-" * 100)
    print(f"{'排名':<4} {'策略名稱':<20} {'Sharpe':<8} {'年收益%':<8} {'波動%':<8} {'回撤%':<8} {'勝率%':<8} {'交易次數':<8} {'最終價值':<10}")
    print("-" * 100)

    for i, strategy in enumerate(results[:10], 1):
        print(f"{i:<4} {strategy['strategy_name']:<20} {strategy['sharpe_ratio']:<8} {strategy['annual_return']:<8} {strategy['volatility']:<8} {strategy['max_drawdown']:<8} {strategy['win_rate']:<8} {strategy['trade_count']:<8} {strategy['final_value']:<10}")
...
def main():
    ...
    elapsed_time = time.time() - start_time
    print(f"✅ 總優化完成，耗時 {elapsed_time:.2f} 秒，總共 {len(results)} 個策略")
