    ...
    def analyze_and_trade(self, input_data: Dict) -> Dict:
        ...
        for i, (signal, price, symbol) in enumerate(zip(signals, close_prices, symbols)):
            ...
            order = {
                ...
                "trading_cost_rate": self.calculate_trading_cost(price * 1000),
        ...
        return {
            "orders": orders,
            "expected_returns": total_expected_return,
            "sharpe_contribution": max(-1, min(1, portfolio_sharpe / self.target_sharpe)),
            "portfolio_sharpe": portfolio_sharpe,
            "total_risk": total_risk,
            "recommendations": recommendations
        }
        
    def _generate_recommendations(self, orders: List[Dict], portfolio_sharpe: float, 
                                balanced_score: float) -> List[str]:
        """生成执行建议"""
        recommendations = []
